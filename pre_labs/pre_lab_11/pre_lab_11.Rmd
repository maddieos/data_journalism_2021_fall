...
**Answer**
There is a .shp file that combines data about every hospital in the US and the number of beds they have. 
```{r, echo=FALSE}
...
**Answer**
There ar 33 columns in this data set.These columns include information about location, such as address, city, state, zip. There are also columns detailing if the hospital has a helipad, trauma center, and the amount of hospital beds.
```{r}
...
**Answer**
According to this data, there are 72 hospital in Maryland. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output below groups the hospitals by type. There are 50 General Acute Care hospitals in MD and 11 Psychiatric hospitals.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The output shows a longitude-latitude grid that plots all of the coordinates for each hospital.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The output looks similar to the output of the hospital shapefile,but there is a new column about geometry. There are 3,233 counties in this data frame.  
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The codeblock filtered the counties dataframe to only include counties that are in Maryland, and then saved the new data frame as "md_counties."
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The codeblock plotted the coordinates of each county and created a map of Maryland with the counties.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output added points to the locations of hospitals in each county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The code block added a color coordinated key to show the number of bed space at hospitals during COVID-19 surges. The color of the points correlates with the color of the key.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output reads in information from the hospitals shapefile. The codeblock filters the data to only show general acute care hospitals in Maryland. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The following output groups hospitals by county and then lists the total amount of hospital beds per county. The output shows that Baltimore City has the most available hospital beds, at 4505.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output displays the population for each Maryland county. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output includes a new category, geometry, which includes the multipolygon data.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This displays two outputs, one with the population data and one with the hospital beds data.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The code block cleans the population data so that it can be joined with the hospital beds data.
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now the population and hospital beds data is all joined together by county.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output replaced the the values listed as NA with 0.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output added a new column that lists the total beds per 100,000 people per county.
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The output is a map of Maryland by county that color cooridinates each county by the total hospital beds per 100k.
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This output added a new color scale, whcih makes it easier to differentiate the key.
...
**Answer**
The output includes the log function which makes the differences between the values more apparent. 
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
