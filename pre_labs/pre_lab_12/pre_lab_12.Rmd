...
**Answer**
The dataframe lists the amount of vaccines per county each day. There are 8581 entries and 13 colummns that list the county name, date, and information about the vaccinations.
```{r}
...
**Answer** There is a row listed as N/A for the county name, but has 202 vaccinations.
...
**Answer**
The information doesn't pertain to any specific Maryland county.
...
**Answer**
The ranks for these days are much higher, meaning that less people are getting fully vaccinated in recent days.
...
**Answer**
Vaccinations peaked between April 2021 and July 2021.
```{r fig.width=9}
ggplot() +
...
**Answer**
"The number of people in Prince George's County fully vaccinated on November 16 decreased by -54.87 percent compared to the week before"
```{r}
...
**Answer**
THe chart shows a line graph that represents the number of cases in each county since the start of COVID.
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
The scale of the chart changed so we can now see the COVID levels for counties with smaller populations.
...
**Answer**
There are 3233 rows in the counties dataframe.
```{r}
...
**Answer**
There are 24 rows in the mdcuonties dataframe.
```{r}
...
**Answer**
The map is showing the amount of cases per Maryland county as of 11/16/2021 by using a color coded key.
```{r}
ggplot() +
