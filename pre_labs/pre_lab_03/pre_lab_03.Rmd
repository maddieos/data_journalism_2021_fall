...
**Answer** It does not show any parsing errors.
...
**Answer** The "name" field is listed as a <chr> data type. The "id" column is listed as a <dbl> data type.
```{r}
...
**Answer**: There are 192959 loans listed under the "NA" franchise name. There are 275 loans listed under the "Subway" franchise name.
...
**Answer**: Under the forgiveness_amount, 135073 loans are listed under "NA".
...
**Answer**: There are 135073 loans where the forgiveness_date is NA. It is the same amount as the forgiveness_amount. 
...
**Answer**: There are 135073 loans where the forgiveness_amount and forgiveness_date are NA. It is the same number as forgiveness_amount and forgiveness_date ran separately. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for 10000000. The smallest loan is for 6.The loan for $6 is suspicious to me, because that is such a small amount to request a loan for. 
```{r}
...
**Answer** There are two businesses with loans under $100. 
```{r}
...
**Answer** All of the amounts align.
...
**Answer** Six rows are in this demonstration data set.
...
**Answer** The "amount" field is a <chr> or character data type.
```{r}
...
**Answer** First, the column headers are inconsistently styled. Second, the amount column is stored as a character and not a number. Third, there's a fully duplicated row in the dataset. 
...
**Answer** The function changes the name of the column "1_id" to "x1_id".
...
**Answer** The error says: "problem with "summarise()" column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument. I think it means that either total_amount or sum_amount are listed under the wrong data type.
...
**Answer** The "amount" column was changed to <dbl> data type. 
...
**Answer** The total amount of loans is 2033179.
```{r}
# cleaning function
total_arnold_md_loans <- cleaned_arnold_md_loans %>%
  summarise(total_amount = sum(amount))
...
**Answer**There are two duplicate rows listed for Staurt Jones LLC.
...
**Answer** Two misspellings of Arnold remain after running this code.
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer**There is one misspelling after running this code.
...
**Answer**There are no misspellings of Arnold after running this code. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
