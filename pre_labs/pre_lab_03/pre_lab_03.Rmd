...
**Answer** The red warning message says: one or more parsing issues, see 'problems()' for details.
...
**Answer** There are 87927 problems.
...
**Answer** There are no longer any parsing errors.
...
**Answer**: There are 192959 loans under NA. There are 275 loans under "Subway".
...
**Answer**: There are 135073 loans where the forgiveness_date is NA. It is the same number as the forgiveness_amount.
...
**Answer**: There are 135073 loans where the forgiveness_date and forgiveness_amount is NA. It is the same amount as the number for forgiveness_amount and forgiveness_date ran separately. 
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for 10000000 dollars. The smallest loan is for 6 dollars. The $6 loan is suspicious because it is such a small amount of money to request for via a loan.
```{r}
...
**Answer** The various fields with amounts align. 
...
**Answer** The book chapter states that one of the problems is that the column headers are inconsistently styled. Second, the amount column is stored as a character, not a number. Third, there is a fully duplicated row in the data set.
...
**Answer** The error states: "Problem with `summarise()` column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument." I think that this means the "amount" column has an incorrect data type.  
...
**Answer** The amount of all loans is 2033179.
```{r}
# cleaning function
total_arnold_md_loans <- cleaned_arnold_md_loans %>%
  summarise(total_amount = sum(amount))
...
**Answer** There are two rows with the same information. They are listed as Stuart Jones LLC. 
...
**Answer**: There are two mispellings of Arnold that remain after runnning this code. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer** There is one mispelling of Arnold that remains after running this code. 
...
**Answer** There are no mispellings of Arnold that remain after running this code. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
