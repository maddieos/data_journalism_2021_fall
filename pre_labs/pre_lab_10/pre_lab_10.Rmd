...
**Answer** Copy the url of the graphic you published here. https://datawrapper.dwcdn.net/NXDgo/1/
...
The code grouped the data by county and then counted the amount of PPP loans per county. The new dataframe shows this.
...
**Answer** Describe what the code below is doing and the output.
The number of counties listed went from 32 to 24. The code eliminates counties that have less than 6 PPP loans.
...
The new FIPS code column combines the state code and the county code to create a 5-digit FIPS code for each county.
...
The new data frame combines the project county name with the FIPS code.
```{r}
...
The previous map was of the entire country now the map only shows counties in Maryland. 
...
**Answer** Describe the changes from the previous map
When you hover over the county with your mouse, it shows the county name instead of the zipcode and the number contains commas. 
...
https://datawrapper.dwcdn.net/1sX1I/1/