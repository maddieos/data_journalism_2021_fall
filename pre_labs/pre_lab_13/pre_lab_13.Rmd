...
**Answer**The dataframe lists each state, the presidential candidate that won in the state, the percent of voters that voted for Trump, the percent of businesses that received PPP loans, and the average amount of money provided by PPP per small business employee in the state. 
```{r}
...
**Answer** The plot shows the relationship between the percent of businesses that recieved PPP loans in each state and the percent of voters that voted for Trump. The plot showed a slight correlation between the two. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The plot displays the relationship between the average amount of money provided by PPP per small business employee in the state and the percentage of voters that voted for Trump in the state. The plot shows a slight correlation between the two, with a slightly upward slope. 
```{r}
...
**Answer** The output showed that the relationship between the states percent of votes for Trump and percent of businesses receiving PPP loans has a moderate and positive relationship. The relationship between states percent of votes for Trump and the the average amount of money provided by PPP per small business employee in the state is positive, but much less strong. 
```{r}
reveal_data %>%
...
**Answer** The output describes the p-value of the correlation. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The output shows a weak r-value and a larger p value. 
```{r}
...
**Answer** The output calculates the overall averages. Trump votes have a higher mean ppp amount per employee and mean percent of businesses recieving ppp loans. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The output displays both means. It also shows the p-valie and t value. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The output describes that this statistic is more meaingful than the previous statistics. 
```{r}
