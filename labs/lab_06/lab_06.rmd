...
md_international_migration <- read_csv("data/tabula-md-statistical-handbook.csv",skip=2,col_names = c("COUNTY","JULY_2010","JULY_2011","JULY_2012","JULY_2013","JULY_2014","JULY_2015","JULY_2016","JULY_2017","JULY_2018","TOTAL"))
```
...
**A1** The Suburban Washington Region has the largest percentage of international migration for the entire time period.
...
**A2** In the state of Maryland, 198996 people migrated to the state between July 2010 and July 2019. The Suburban Washington Region recieved the largest percentage of migrants during this time period. 
...
**A3** Cecil County had the largest percent change for international migration between July 2018 and July 2017.

```{r}
# Write the code to produce your answer here
md_international_migration %>%
  mutate(
    percent_change18=(JULY_2018-JULY_2017)/JULY_2017
  )%>%
  arrange(desc(percent_change18))

```

**Q4** What's your best guess as to why these changes occurred, and in those places in particular?
**A4** A possibility for this change is due to the Trump Era immigration laws. The percent change from 2017 to 2018 was -1.27%. The Trump Era immigration laws made it harder for migrants to come into the country, which could be the cause for this decrease in migration.  
...
```{r}
# Write the code to load the CSV file here
domestic_migration <- read_csv("data/tabula-md-statistical-handbook2.csv",skip=2,col_names = c("COUNTY","JULY_2010","JULY_2011","JULY_2012","JULY_2013","JULY_2014","JULY_2015","JULY_2016","JULY_2017","JULY_2018","TOTAL"))
```
...
**A6** 18 regions and jurisdictions had net positive migration for July 2017, July 2018, and July 2019.

```{r}
# Write the code to produce your answer here
domestic_migration <- domestic_migration %>%
  mutate(
    net_positive = (JULY_2016+JULY_2017+JULY_2018)
  )%>%
arrange(desc(net_positive))
...
**A7** Frederick County has the largest net positive migration for July 2017, 2018, and 2019. It stands out that most of these counties are on the Eastern Shore, which is a predominately low population area. I wonder why so many people are opting to move to these areas, rather than say Baltimore City, Baltimore County, or near the DC metro area. 
