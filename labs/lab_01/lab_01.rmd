...
**A1** [write your answer here] 33,703 rows, 63 columns
...
**A4** Kanawha County has a higher population than Writ County, therefore there would be a need for more loans.
...
**A5** Greenbrier Hotel Corporation got the state's largest PPP loan. They received $8912202.Greenbrier Hotel Corporation is a vacation resort. The loan retained 500 jobs with the loan.
...
**A6** Jim Justice, the governor of West Virginia, owns Greenbrier Hotel Corporation. The source for this information is from the Greenbrier website.
...
**A7** The average (Mean) loan was $77338.08. This information can be used to gage how much money was lended on average and to gage how much financial aid businesses needed. 
```{r}
# Write the code to produce your answer here
wv_ppp_loans %>%
  summarise(
...
**A8** Kanawha County had the largest count and amount of loans out of all of the counties, but also had the smallest minimum loan amount out of all of the counties. Unexpectedly,Kanawha County did not have the largest maximum loan amount out of all of the counties, even though it had the largest amount of loans.
```{r}
# Write the code to produce your answer here
wv_ppp_loans %>%
  group_by(project_county_name) %>%
