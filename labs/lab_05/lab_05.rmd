...
**A1.**
126 E Burke St has the most loans. There are 6 loans at that street address. The names of the businesses at that address are Drew Holdings LLC, Brix27 LLC, Abraham Ashton, Hub Co-Op LLC, and Ronin Properties LLC. 

```{r}
zip_25401_loan_sample_clean <- read_csv("data/zip_25401_loan_sample-csv.csv")

zip_25401_loan_sample_clean %>%
  group_by(address)%>%
  summarise(
    count= n()
  ) %>%
...
**A2.**
Considering that Hub Co-Op LLC is a community work space called "The Hub." Since it is a community work space, many new businesses may be running out of this work space, which is why there are a few businesses listed under the same address. Also, one person, Abraham Ashton, is connected to all of the businesses.
...
**A3.**
Martinsburg has the highest number of loans. Full-Service Restaurants is the industry title with the most loans. 
```{r}
Berkeley_loans <- lab_05 %>%
  filter(project_county_name == "BERKELEY")
  write_csv(Berkeley_loans, "data/Berkeley_loans.csv")
  
Berkeley_loans_clean <- read_csv("data/Berkeley_loans-csv.csv")

Berkeley_loans_clean_naics_code <-Berkeley_loans_clean %>% left_join(naics_codes) 
  
Berkeley_loans_clean_naics_code %>%
  group_by(city)%>%
 summarise(
    count= n()
  ) %>%
...
**A4.** 
During the COVID-19 pandemic, many restaurants were forced to close due to them being non-essential businesses. Also due to the pandemic, less people were opting to go out to eat, bringing in less business. 
...
**A5.** Cottle Christi L LLC is the name of the business with the highest approved loan. The loan was for 280434.00. It was approved 02/17/2021. The loan helped retain 94 jobs. The restaurant is known as Kitzie's Restaurant and Lounge. The restaurant has two locations. One of its locations, the Inwood location, closed, but in 2019 before the pandemic. 
